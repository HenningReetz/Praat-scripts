...
#  	Version 2.3.3, Henning Reetz, 27-may-2022, added max_result_header computation, resets "?" from result files to ""
#  	Version 2.3.4, Henning Reetz, 28-may-2022, corrected handling of label list
#  	Version 2.3.5, Henning Reetz, 22-apr-2024, adjusted default pitch settings to new praat version
#  	Version 2.3.6, Henning Reetz, 01-may-2024, improved final message
#
#§§§ does not inspect more than 1 label! check other scripts as well!
#	Tested with Praat 6.4.0
##
...
result_header$# = { file_string$, label_string$, start_string$, duration_string$ }
max_result_header = 4
...
# entertain user
			...
			if (seconds - last_seconds > 1)
				writeInfo: "'i_file' files of 'nr_files' scanned."
				last_seconds = seconds
			endif
...
# no files to process
		...
		printline 'newline$'No files to process. Program aborted.
		call RemoveObjects
		exit
	endif
...
# reset parameters if required
		if (default)
			...
			Pitch settings: 75, 500, "Hertz (logarithmic)", "filtered autocorrelation", "automatic", 0, 0
