...
#	This script creates a file "Inspect_progress_file_V03.txt" to handle the skipping of .wav/.TextGrid files
...
#  	Version 2.2.1, Henning Reetz, 29-jul-2021, "window = 0" if progress file exists 
#
...
### 2 >>>
# comments, notes and judgments switches and file names (set flags to '1' to enable them)
...
#	(note that these names will be overwritten in case there is a progress-file)
notes_flag = 1
notes_file$ = "001_Inspect_"+date_time$+"_notes.txt"
notes_header$ = "file" + tab$ + "start" + tab$ + "end" + tab$ + "cursor"
...
progress_file_name$ = "'support_directory$'000_Inspect_progress_file_V03.txt"
...
### Add global parameters to progress_file
	...
	if (nr_rows)
		...
		row += 1
		Insert row: row
		Set numeric value: row, "state", 9
		if (notes_flag)
			Set string value: row, "file", "'result_directory$''notes_file$'"
		else
			Set string value: row, "file", " "
		endif
...
# create comments and judgments files if necessary and write headers
	...
	if (notes_flag)
		appendFileLine: notes_file$, notes_header$
	endif
	if (comments_flag)
...
# get global parameters
	...
	notes_file$ = Get value: 8, "file"
	notes_flag = (notes_file$ <> " ")
	window = 0
endif
...
#
			...
			info$ = Editor info
			start_time = extractNumber (info$, "Window start: ")
			end_time = extractNumber (info$, "Window end: ")
# close editor window
	Close
...
# handles notes
	if (notes_flag and note_this_down)
		start$ = fixed$(start_time,4)
		cursor$ = fixed$(cursor_time,4)
		end$ = fixed$(end_time,4)
		info$ = "'base_name$''tab$''start$''tab$''end$''tab$''cursor$'"
		appendFileLine: notes_file$, info$
	endif
...
printline Done. 'nr_handled_items' items checked.
