...
#	Vers. 6.1.1, Henning Reetz, 23-feb-2021	remove double commas in label$
#	Vers. 6.1.2, Henning Reetz, 20-may-2021	output intensity_mean always with ReportValue
#	Vers. 6.2.0, Henning Reetz, 10-aug-2022	'med' - median-subtraction added
#	Vers. 6.2.1, Henning Reetz, 11-aug-2022	other computation methods added
#	Vers. 6.2.2, Henning Reetz, 05-sep-2022	corrected form header
#
...
form Pitch parameters (Vers. 6.2.2):
		word Tier 0
	...
	comment (t)ime of measurement, mean (s)ubtraction, (med)ian subtraction, (z)-scores?
...
#	object identifier (e.g. pitch_obj) to 0.
###
...
#		(time) (intensity)
#		for pitch units
#			pitch (mean subtraction) (median subtraction) (z-score)
#		endfor
###
...
# Get quantiles if required
							if (nr_quantiles)
								for i_quantile to nr_quantiles
									...
									if ((quantile_value = 0.5) and cmed_request)
										median_pitch = quantile
									endif
...
# Use the same mechanisme for contours of times, values, means & medians subtraction, and z-score
...
# the quantile values are stored in quantile_obj and used during computation from there
...
# force mean request (to be able to do a substraction)
			im_request = 1
# median subtraction requested?
		elsif (item$ = "med")
			cmed_request = 1
# force median request (to be able to do a substraction)
			if (nr_quantiles = 0)
...
# next loop is only entered if any time point measurements (incl. center and edges ) are requested
...
# no sound object loaded yet?
		...
		if (obj$ = "intensity_obj")
			...
			if (method$ = "ac")
				...
				pitch_obj = 'np_string$' To Pitch (cc): step_rate, low_F0, 15, "no", 0.03, 0.45, 0.01, 0.35, 0.14, high_F0
			elsif (method$ = "spinet")
				pitch_obj = 'np_string$' To Pitch (SPINET): step_rate, 0.04, low_F0, 70, 5000, 250, high_F0, 15
			elsif (method$ = "shs")
				pitch_obj = 'np_string$' To Pitch (shs): step_rate, low_F0,  15, 1250, 15, 0.84, high_F0, 48
			else
				printline Illegal pitch algorithm selection in procedure SelectObject: 'method$'. Program aborted.
				exit
			endif
		elsif (obj$ = "wav_obj")
		else
...
# report subtracted median
# (pitch values are already computed at this time and stored in variables like pitch1, pitch2…)
		elsif (x$ = "med")
			for i_pos to number_of_measurements
				if (median_pitch <> undefined)
					pitch_med = pitch'i_pos' - median_pitch
				else
					pitch_med = undefined
				endif
				call ReportValue pitch_med 2
			endfor
...
# (pitch values are already computed at this time and stored in variables like pitch1, pitch2…)
			...
			for i_pos to number_of_measurements
				if ((mean_pitch <> undefined) and (stdev_pitch <> undefined))
...
procedure ReportAnalysisParameter
	...
	if (method$ = "ac")
		...
		help$ = "Cross-Correlation"
	elsif (method$ = "spinet")
		help$ = "Spatial PItch NETwork"
	elsif (method$ = "shs")
		help$ = "Sub-Harmonic Summation"
	else
		help$ = method$
	endif
	out_line$ += "Computation method: 'help$''newline$'"
