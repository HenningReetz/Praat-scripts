...
form Pitch parameters (Vers. 6.0):
		integer Tier 1
...
###
# report interval data if all reuirements are fulfilled
###
...
# (for voicing computation, any init can be used)
##@@						 if (m_request or nr_quantiles or (number_of_measurements > 1))
							unit$ = "Hertz"
							call ReportVoicing
##@@						endif
...
### contour data requested (i.e. number_of_measurements <> 0)
						if (number_of_measurements)

### report time data only once (if at all
							if (t_request)
								call ReportContour t
							endif


### Now report contour data.
...
#
# interval should not be reported (l_flag = 0), but the user might want to report skipped intervals
#
...
if ((nr_grid <> nr_wav) and (nr_grid <> 0))
	...
	printline But 'nr_grid' TextGrid files not found.
endif
...
###
# Procedure to open sound- and TextGrid file (or generate dummy TextGrid if whole files should be analysed)
###
...
###
# Procedure to prepare output line (genaret first part of line)
###
...
# report times of measurement
		if (x$ = "t")
			t_mid = mid
			for i_pos to number_of_measurements
				fileappend 'result_file_name$' 'sep$''t_mid:04'
				t_mid += dur_step
			endfor
		endif
...
###
# Report the amount of voicing in an interval in percentage or interval length
###
...
procedure ReportValue v l
	if (v <> undefined)
		if (l = 0)
			...
			fileappend 'result_file_name$' 'sep$''v:2'
		elsif (l = 3)
