...
form Intensity parameters (Vers. 6.0):
	...
	comment <label>, <list>.txt, '.' (= only labelled), or empty (= all)
		sentence Label .
	comment (m)ean, <list of quantiles in %> of intervals
		sentence Interval_parameters m,0,50
...
# get number of measurements (an empty integer field in the from window cannot be left empty, 
# so we convert a string to a nimber here)
