...
#	Vers. 5.0.0, Henning Reetz, 02-nov-2020; synchronizing with other scripts
#	Vers. 5.0.1, Henning Reetz, 02-nov-2020; better final message
#	Vers. 5.0.2, Henning Reetz, 23-feb-2021; remove double commas from label$
#	Vers. 5.0.3, Henning Reetz, 15-may-2023; replaced boolen 'false' by '0'
#
...
form Duration (Vers. 5.0) parameters:
	...
	comment Handling of skipped labels
		boolean Report_missing 0
		word Missing_value_symbol NA
		boolean Fast_processing no
endform
...
# report only time of point
						...
						fileappend 'result_file_name$' 'base_name$''sep$''interval_label$''sep$''begin_interval:4''missing_value$''newline$'
					endif

# interval/point does not match criteria to be reported
				elsif (report_missing)
					fileappend 'result_file_name$' 'base_name$''sep$''interval_label$''missing_value$''missing_value$''newline$'
...
# report files without matching items if requuested
			if (report_missing and !nr_measured_intervals)
				fileappend 'result_file_name$' 'base_name$''missing_value$''missing_value$''missing_value$''newline$'
			endif
# entertain user
			if (type$ = "interval")
				tot_intervals += nr_measured_intervals
			elsif (type$ = "point")
				tot_points += nr_measured_intervals
			else
				printline Impossible type$: 'type$'. Script aborted.
				exit
...
# requested tier was beyong maximal number of tiers
			...
			if (!user_feedback)
				print 'newline$'Handling 'base_anme$''space$'
			endif
...
# remove file from object list
	...
	if (user_feedback)
		...
		if (tier)
			print with 'nr_measured_intervals' 'type$''space$'
		endif
		printline finished. ('percent:2'% processed).
	endif
...
if (tier = 0)
	...
	if (!tot_intervals and !tot_points)
		printline 'newline$''nr_files' files processed but no intervals or points found.
	elsif (tot_intervals and tot_points)
		printline 'newline$''nr_files' files with a total of 'tot_intervals' intervals and 'tot_points' points processed.
	elsif (tot_intervals)
		printline 'newline$''nr_files' files with a total of 'tot_intervals' intervals processed.
	elsif (tot_points)
		printline 'newline$''nr_files' files with a total of 'tot_points' points processed.
	else
		printline This is simply impossible.
		exit
	endif
...
# create header, dummy and missing data lines (first part is always there, so no missing data needed)
	...
	if (tier = 0)
		...
		skip_line$ += "'missing_value$''missing_value$'"
	endif
	if (duration_in_ms)
